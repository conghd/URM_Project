...
const NameInput = forwardRef(({handleTextChange, handleSubmitEditing,
  ...
  const validateName = (text) => {
    ...
    if (/^\s|\s$/.test(text)) {
      return "Your name cannot contain spaces at the start or end";
    }
    if (text.length < 6 || text.length > 30) {
      return "Your name must be between 6 and 30 characters long.";
    }
    // let reg = /^[A-Za-z]([[\.-]?[A-Za-z0-9]+)*$/;
    const reg = /([A-Z][a-z.]*[ -])+[A-Za-z]+/;
    const msg = (reg.test(text)) ? "":
    "Your name must be capitalized. Only letters(a-z, A-Z), spaces, full-stops are allowed";
    return msg;
  };
